...
type RootPackage = { readonly version?: unknown };
...
describe('ECO ChatGPT/Codex plugin package', () => {
  it('keeps ECO identity and version synchronized with the v4.31+ runtime', async () => {
    const manifest = JSON.parse(await readFile(path.join(root, '.codex-plugin', 'plugin.json'), 'utf8')) as EcoPluginManifest;
    const rootPackage = JSON.parse(await readFile(path.join(root, 'package.json'), 'utf8')) as RootPackage;
    const pluginInterface = manifest.interface ?? {};

    expect(manifest.name).toBe('eco');
    expect(manifest.version).toBe(rootPackage.version);
    expect(pluginInterface.displayName).toBe('ECO');
    expect(manifest.description).toMatch(/headless MCP/i);
    expect(pluginInterface.longDescription).toMatch(/headless stdio MCP/i);
    expect(manifest.skills).toBe('./skills/');
    expect(pluginInterface.capabilities).toEqual(expect.arrayContaining(['Interactive', 'Read', 'Write']));

    for (const field of ['composerIcon', 'logo', 'logoDark']) {
      const relative = String(pluginInterface[field] ?? '').replace(/^\.\//, '');
